...
| REV-01 | Delete user's seed from PostgreSQL and Redis on revocation | Verified end-to-end (in-memory backend) | — | `DELETE /v1/users/{user_id}` tested in `test_routes.py` (`test_revoke_then_authenticate_fails`, `test_revoke_unknown_user_is_idempotent`); revoking makes a previously-authenticating vector fail (401), through a real HTTP call. `PostgresRedisStorageBackend`'s both-stores-must-succeed-or-fail semantics remain untested against live services, and the partial-failure path (one store succeeds, the other doesn't) still has no test at all, in-memory or real, since the in-memory fake has no way to fail. |
