...
| **WIDER FACE** | ✅ Used, R&D-scope only | CC BY-NC-ND 4.0 (non-commercial, no-derivatives) -- same R&D-only scope applies. No per-identity labels at all, and it's a detection dataset (cluttered scenes, not single-face portraits) -- cropped to face bounding boxes and filtered for occlusion/blur/size before use; see `colab_train_stage1.ipynb`. |
...
- `colab_train_stage1.ipynb` -- the actual training pipeline: downloads FairFace + CelebA + WIDER FACE, generates labels, trains, exports to ONNX, INT8-quantizes, verifies. Built for Google Colab (free GPU, avoids fighting local CUDA/torch version conflicts -- see STATUS.md for what went wrong trying to do this locally). Clones the repo inside the notebook so it trains against the exact committed `feature_targets.py`, not a pasted-in copy that could drift.
